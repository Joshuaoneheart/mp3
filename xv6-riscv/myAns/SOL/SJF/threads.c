...
void schedule(void){
    ...
    {
        int exited = current_thread->is_exited, yielded = current_thread->is_yield;
        if (current_thread->is_exited || current_thread->is_yield) {
            current_thread = current_thread->next;
            struct thread *it = current_thread->next, *next = current_thread;
            while (it != current_thread){
                if( (it->remain_execution_time < next->remain_execution_time) 
                || (it->remain_execution_time == next->remain_execution_time && it->ID < next->ID ) ){
                    if(yielded || (exited && it->is_exited == 0))
                        next = it;
                }
                it = it->next;
            }
